...
class DeleteAssignmentUseCaseTest {

  private lateinit var repo: FakeDeleteAssignmentRepo
  private lateinit var useCase: DeleteAssignmentUseCase

  @Before
  fun setup() {
    repo = FakeDeleteAssignmentRepo()
    useCase = DeleteAssignmentUseCase(repo)
  }

  @Test
  fun `deletes assignment by id`() = runTest {
    useCase(42)

    assertEquals(1, repo.deletedIds.size)
    assertEquals(42, repo.deletedIds[0])
  }
  ...
  fun `delete non existent id does not throw`() = runTest {
    ...
    assertEquals(1, repo.deletedIds.size)
    assertEquals(9999, repo.deletedIds[0])
  }

  class FakeDeleteAssignmentRepo : IPracticeRepository {
    ...
    override suspend fun deleteAssignmentById(id: Int) { deletedIds.add(id) }
  }
}
